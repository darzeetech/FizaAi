...
const CollectiveCard: React.FC<CollectiveCardProps> = ({ item, onShowInfoChange, onRemove }) => {
    ...
    return (
        <motion.div
            className={`relative flex flex-col md:flex-row bg-white shadow-md hover:shadow-lg mx-auto w-full max-w-[95%] md:mt-2 ${showInfo ? ' bg-black bg-opacity-25' : ''
        ...
        >
            ...
            >
                        ...
                        <div className="flex justify-center gap-3 md:hidden absolute bottom-6 left-1/2 -translate-x-1/2 z-[100]">
                            {item.images.map((_, index) => (
                                <button
                                    ...
                                    className={`w-3 h-3 rounded-full  ${index === currentImageIndex ? 'bg-gray-500' : 'bg-gray-400'
                                        }`}
                                />
                            ))}
                        </div>
                ...
                <div className="md:flex items-center gap-5 mt-3 hidden ">
                    <span
                        className={`${item.platForm === 'DARZEE'
